...
RUN /bin/bash -c "git clone https://github.com/meetecho/janus-gateway.git \
                  && cd janus-gateway \
                  && sh autogen.sh \
                  && ./configure --prefix=/opt/janus --enable-plugin-lua --enable-websockets --enable-websockets-event-handler \
                  && make -j 8 \
                  && make install \
                  && make configs \
                  && cd .. \
...
COPY --from=builder /lib64/libssl.so.1.1 .
COPY --from=builder /lib64/libssl.so .

COPY --from=builder /lib64/libcrypto.so.1.1 .
COPY --from=builder /lib64/libcrypto.so .
