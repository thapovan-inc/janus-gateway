...
RUN apt update && apt install -y build-essential libmicrohttpd-dev libjansson-dev libssl-dev libsrtp-dev libsofia-sip-ua-dev libglib2.0-dev libopus-dev libogg-dev libcurl4-openssl-dev liblua5.3-dev libconfig-dev pkg-config gengetopt libtool automake git python3-pip && pip3 install meson
RUN mkdir -p /usr/src
