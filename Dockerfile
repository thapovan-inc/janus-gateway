...
RUN apt update && apt install -y build-essential libmicrohttpd-dev libjansson-dev libssl-dev libsrtp2-dev libsofia-sip-ua-dev libglib2.0-dev libopus-dev libogg-dev libcurl4-openssl-dev liblua5.3-dev libconfig-dev pkg-config gengetopt libtool automake git python3-pip cmake && pip3 install meson ninja
RUN mkdir -p /usr/src
